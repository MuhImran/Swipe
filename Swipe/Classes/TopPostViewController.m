...
#import "TopPostViewController.h"
...
@implementation TopPostViewController
...
- (NSArray*)   printAddressBook   
{
    ...
    for (peopleCounter = 0;peopleCounter < [arrayOfAllPeople count]; peopleCounter++){
        ...
        // user.imageData =  (NSData*)ABPersonCopyImageDataWithFormat([thisPerson objectAtIndex:index], kABPersonImageFormatThumbnail);
...
-(void) dataForOtherTab:(UITableViewCell *)hlcell:(NSIndexPath *)indexPath
{
	...
	while(i<n)
    {
		...
		for(j=0; j<4;j++)
        {
			...
			if (![imageCaches imageFromKey:person.imgURL])
		    {
                //       
