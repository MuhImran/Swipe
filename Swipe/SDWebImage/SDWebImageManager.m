...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate:(NSMutableDictionary *)_dic
{
    userInfo = _dic;
   // [self downloadWithURL:url delegate:delegate options:0];
    
      [self downloadWithURL:url delegate:delegate options:0:_dic];
    
}
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options:(NSMutableDictionary *)_userInfo
{
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:delegate, @"delegate", url, @"url", [NSNumber numberWithInt:0], @"options",_userInfo,@"userInfo",nil];
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    NSMutableDictionary *userinfo = [downloader.userInfo objectForKey:@"userInfo"];
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                else  if ([delegate respondsToSelector:@selector(webImageManager:didFinishWithImage2:)])
                {
                    [delegate performSelector:@selector(webImageManager:didFinishWithImage2:) withObject:image withObject:userinfo];
                }  
