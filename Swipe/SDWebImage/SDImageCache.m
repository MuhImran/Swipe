...
{
   // image = [commonUsedMethods imageWithImage:image];
    data = UIImageJPEGRepresentation(image, 0.5);
    NSLog(@"BEFORE==>DOwnloaded Image Bytes are:%d",[data length]);
    if (!image || !key)
    {
        return;
    }
...
- (void)removeImageFromMemoryWithKey:(NSString *)key
{
    ...
    NSLog(@"dictionary count before deletion : %d",[memCache count]);
    [memCache removeObjectForKey:key];
    NSLog(@"dictionary count after deletion : %d",[memCache count]);
   // [[NSFileManager defaultManager] removeItemAtPath:[self cachePathForKey:key] error:nil];
}
